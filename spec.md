...
API extraction expands local dumi `<embed src="...">` references before parsing tables, so shared API files (for example Tooltip/Popover/Popconfirm props) are included. Embedded shared-props files are classified as common API even when the parent document uses only an HTML comment as its label. When a shared table repeats a component-specific property, non-empty component-specific fields take precedence, missing fields are filled from the shared definition, and the shared definition is not appended as another row. Existing duplicate rows in component-specific tables are preserved and paired with localized rows by occurrence. Both `Default` and `Default value` table headers are recognized, while `-` version placeholders are omitted from `since` metadata.
